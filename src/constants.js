export const BASE_URL = 'http://localhost:8765'

export const AUTH_URL = '/USERSERVICE/api/auth'
export const USER_URL = '/USERSERVICE/api/users'
export const ADMIN_URL = '/USERSERVICE/api/admin'
export const CASE_URL = '/CASEMICROSERVICE/api/case'
export const DOC_URL = '/CASEMICROSERVICE/api/document'
export const EMPLOYEE_URL = '/USERSERVICE/api/employee'
export const LAWYER_URL = '/USERSERVICE/api/lawyer'
export const COUNCIL_URL = '/USERSERVICE/api/council'
export const ROLE_URL = '/USERSERVICE/api/roles'
export const CENSUS_URL = "/APISERVICE/api/proxy"
export const EMAIL_URL = '/EMAILSERVICE/api/mail/send'
export const NDI_URL = "/APISERVICE/api/ndi"